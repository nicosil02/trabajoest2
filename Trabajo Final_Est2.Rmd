...
```
En esta ocasión se está haciendo un corte en la variable Q82 "Confianza en Organización Regional relevante" debido a su naturaleza ya que introduce variables adicionales como Q82A y Q82B para poder preguntar acerca de las organizaciones locales relevantes específicas (Ejemplo, African Union (AU), European Union (EU), Caribbean Community (CARICOM), Mercado Común del Sur (MERCOSUR), etc). Para evitar más datos perdidos, dado que no todas los países son parte de las mismas regiones, vamos a evitar la especifidad que abarca estas variables y solo concentrarnos en la totalidad del Q82. 

Asimismo, las variables con las que trabajeremos son numéricas ordinales. 
...
A continuación presentaremos las 26 variables que refieren a la confianza en las instituciones
...
De acuerdo con el análisis del gráfico de sedimentacion, ubicamos que el codo se encuentra en el 2. Al verificar en el otro cuadro se recomienda que se utilice 1 factor.

Como nos recomienda solo 1 factor, vamos a asegurar aún más con los autovalores ya que no sería adecuado agrupar en solo 1 factor las variables de instituciones que no todas comparten la misma naturaleza.
...
Al observar los autovalores, vemos que hay 4 autovalores que superan a 1. Entonces son 4 los factores que podríamos realizar.

Tras observar los autovalores y el gráfico de sedimentación podemos concluir que los más apropiado sería distribuir las variables en 4 factores. Elegir un número mayor a este, implicaría arriesgarnos a una menor exactitud y efectividad de los factores para responder a nuestra pregunta de investigación.
...
```{r}
factorial_exp1 <- fa(subdata_1,nfactors = 4,rotate="varimax", fm="minres")
factorial_exp1
```
...
A pesar de no contar con errores en la asignación de factores, las asignaciones propias de R carecen de sentido en algunos casos. Asimismo, considerando conocimiento desde la Ciencia Política, se puede construir una distribución de patrones que organice adecuadamente la confianza institucional.  EXPLAYAR 
...
F1 Confianza en las OI

F2 Confianza en los medios de comunicación

F3 Confianza en las Instituciones Políticas y Legales

F4 Confianza en las Instituciones de seguridad

F5 Confianza en Instituciones Educativas y Espirituales

56 Confianza en Instituciones Sociales
...
Los niveles de confianza se clasifican de la sigueinte manera:
