...
Explicar que vamos a corroborarlo
...
```{r}
Modelo_confir3 = "FAC1 =~ nacionalidad + preligion + rconocidos + conocidos
                FAC2 =~ barrio + familia"
Modelo_confir3
```

Lo que se realizaría es indicar el nombre de cada facrtor, y qué variables las integran, solo indicamos el nombre de estas variables, en el siguiente comando indicaríamos la base (editar)

```{r}
modelo_inter=cfa(Modelo_confir3, data=subdata_2)
summary(modelo_inter,fit.measures=F)
```
